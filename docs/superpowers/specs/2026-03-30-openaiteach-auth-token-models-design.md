...
| 文件 | 改动 |
|------|------|
| `vite.config.ts` | 加 `/api` 和 `/v1` 双代理 → `https://openaiteach.com` |
...
| 文件 | 原因 |
|------|------|
| `src/components/modals/ApiSettingsModal.tsx` | 被 SettingsModal 完全替代，需删除并清理所有 import |
...
**注意：** `turnstile=` 为空字符串，openaiteach.com 接受空验证码（无需第三方 Turnstile SDK）。

**session 存储字段：**
```ts
SessionPayload {
  ok: boolean
  sessionToken: string   // "cookie-session:{userId}" 占位符（无真实 token，依赖 Cookie）
  userId?: string
  username?: string
}
```
登录成功后，浏览器 Cookie 由服务器设置并自动托管（HttpOnly）。`sessionToken` 仅为前端标识登录状态用，不发往服务器。
...
- [刷新列表] 按钮
- 分隔线 + 手动输入 `sk-xxx` 框 + [保存] 按钮（保存前自动 trim，若已有 `sk-` 前缀不重复添加）
- 底部显示当前使用的 Token 名字
- [测试连接] 按钮（调 `/api/user/self`）
- Token 列表固定拉取前 20 条（`p=0&size=20`），超出部分提示"在网站管理更多 Token"
...
节点发起请求
  ...
  → 优先级：selectedTokenValue（手动或账号 Token）> Cookie 会话
  → 如果有 sk-xxx: Authorization: Bearer sk-xxx，同时 credentials: include
  → 如果无 token: 不加 Authorization，仅 credentials: include（依赖 Cookie）
  → 请求目标（开发）: /v1/... → 由 Vite 代理转发至 https://openaiteach.com/v1/...
  → 请求目标（生产）: https://openaiteach.com/v1/...
```
...
| 用途 | dev 路径 | prod 路径 |
|------|---------|---------|
| 后台 API（登录/Token）| `/api/...` | `https://openaiteach.com/api/...` |
| AI 推理（模型调用）| `/v1/...` | `https://openaiteach.com/v1/...` |

`env.ts` 导出两个基址：`apiBaseUrl` 和 `aiBaseUrl`。

### 开发代理（仅本地）
```
vite.config.ts:
  /api  →  https://openaiteach.com  (changeOrigin: true)
  /v1   →  https://openaiteach.com  (changeOrigin: true)
```
两个代理均转发，避免直连线上触发 CORS。

### 401 / 会话失效处理
```
任意 /api 或 /v1 请求返回 401
  → 清空 session-store（setSession(null)）
  → 清空 token-config-store（reset()）
  → 跳回登录页
  → 显示提示："登录已过期，请重新登录"
```
...
- localStorage 只存：`selectedTokenValue`（sk-xxx 明文）+ 勾选的模型 ID 列表 + session 标识（userId/username，不含密码）
- HttpOnly Cookie 由浏览器自动托管，前端不可读写
- 调试用的 `127.0.0.1:7270` ingest 代码（从旧项目迁移时）全部删除，从 `auth-client.ts`、`http.ts`、`token-client.ts` 中清除
- 跨域说明：TwitCanva 与 openaiteach.com 为不同域名，开发时依赖 Vite 代理，生产部署时需 openaiteach.com 服务端配置 `Access-Control-Allow-Origin` 和 `Access-Control-Allow-Credentials: true`（后端已支持）
...
- 登录 openaiteach.com 账号后能进入主画布（session 写入 localStorage，刷新页面无需重新登录）
- 设置 → Token Tab → 刷新列表 → API 调用成功，列表渲染正确（有 Token 时显示列表，无 Token 时显示空列表并提示"请在网站创建 Token"）
- 设置 → 模型 Tab → 显示 59 个模型，勾选/取消后节点下拉框同步更新
- 401 响应时自动跳回登录页并显示提示
- 深/浅模式切换，登录页和设置弹窗颜色跟随 canvasTheme 变化
