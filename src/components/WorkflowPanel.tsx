                    ...
                    {loading && activeTab === 'my' ? (
                        ...
                        workflows.length === 0 ? (
                            <div className="flex items-center justify-center h-40 text-neutral-500">
                                {t('workflowPanel.noWorkflowsFound')}
                            </div>
                        ) : (
                                ...
                                {workflows.map(workflow => (
                                    ...
                                    >
                                        ...
                                        <div className="aspect-[4/3] bg-gradient-to-br from-neutral-800 to-neutral-900 flex items-center justify-center relative overflow-hidden">
                                            ...
                                            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                                ...
                                                <button
                                                    ...
                                                    title={t('workflowPanel.editCover')}
                                                >
                                                ...
                                                <button
                                                    ...
                                                    title={t('workflowPanel.deleteWorkflow')}
                                                >
                                        ...
                                        <div className={`p-3 ${isDark ? 'bg-neutral-900/50' : 'bg-neutral-100/90'}`}>
                                            <h3 className={`font-medium text-sm truncate ${isDark ? 'text-white' : 'text-neutral-900'}`}>{workflow.title || t('workflowPanel.untitled')}</h3>
                                            <p className={`text-xs mt-0.5 ${isDark ? 'text-neutral-500' : 'text-neutral-600'}`}>
                                                {t('workflowPanel.nodesCount', { count: workflow.nodeCount })}
                                            </p>
                                        </div>
                                    </div>
                                ))}
                        ...
                        publicWorkflows.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-40 text-neutral-500 gap-2">
                                ...
                                <p>{t('workflowPanel.noPublicWorkflows')}</p>
                                <p className="text-xs text-neutral-600">{t('workflowPanel.addWorkflowJsons')}</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-3 gap-4">
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-[#1a1a1a] border border-neutral-700 rounded-2xl p-6 w-[340px] shadow-2xl">
                        <h3 className="text-lg font-semibold text-white mb-2">{t('workflowPanel.deleteTitle')}</h3>
                        <p className="text-neutral-400 text-sm mb-6">
                            {t('workflowPanel.deleteConfirm')}
                        </p>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-[#1a1a1a] border border-neutral-700 rounded-2xl p-6 w-[500px] max-h-[500px] shadow-2xl flex flex-col">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-lg font-semibold text-white">{t('workflowPanel.selectCoverImage')}</h3>
                            <button
